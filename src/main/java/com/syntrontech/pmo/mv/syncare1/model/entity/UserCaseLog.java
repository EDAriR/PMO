...
import com.syntrontech.syncare.model.transfer.UserCaseLogTO;

@Entity
@Table(name = "user_case_log")
public class UserCaseLog implements ObjectConverter<UserCaseLogTO>, Serializable {
	...
	@Override
	public UserCaseLogTO convert(boolean relation) {
		UserCaseLogTO userCaseLogTO = new UserCaseLogTO();

		userCaseLogTO.setCaseNote(caseNote);
		userCaseLogTO.setCaseStatus(caseStatus);
		userCaseLogTO.setCaseStatusDisplay(caseStatusDisplay);
		userCaseLogTO.setLogId(logId);
		userCaseLogTO.setUpateDate(upateDate);
		userCaseLogTO.setUserId(userId);

		return userCaseLogTO;
	}
}
