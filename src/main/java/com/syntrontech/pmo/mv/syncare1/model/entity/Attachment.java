...
import com.syntrontech.syncare.model.transfer.AttachmentTO;
...
public class Attachment implements ObjectConverter<AttachmentTO>, Serializable {
	...
	@Override
	public AttachmentTO convert(boolean relation) {
		AttachmentTO to = new AttachmentTO();
		to.setId(getId());
		to.setAppType(getAppType().toString());
		to.setOfileName(getOfileName());
		to.setNfileName(getNfileName());
		to.setFilePath(getFilePath());
		to.setFileExt(getFileExt());
		to.setFileSize(getFileSize());
		to.setContentType(getContentType());
